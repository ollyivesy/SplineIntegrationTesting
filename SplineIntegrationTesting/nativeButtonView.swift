...
struct nativeButtonView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            WebView(webView: $webView, isLoaded: $isWebViewLoaded)
    ...
    func setTestBoolToTrue() {
        let script = "setTestBoolTrue();"
        webView?.evaluateJavaScript(script) { (result, error) in
            DispatchQueue.main.async {
                if let error = error {
                    self.resultMessage = "Error: \(error.localizedDescription)"
                } else {
                    self.resultMessage = "TestBool set to True"
                }
            }
        }
    }
